...
namespace SystemInfo {
    ...
    double getCpuUsage() {
        //Intiliaze PDH for system-wide CPU monitoring via PDH
        if(!pdhInitialized) {
    ...
    QVector<DiskInfo> getDiskInfo() {
        ...
        for (const QStorageInfo &storage : volumes) {
            ...
            if (disk.totalSpace > 0) {
                disk.usagePercentage = (disk.usedSpace * 100)/disk.totalSpace;
                disks.append(disk);
            }
        }
            ...
            if (pIfTable != nullptr) {
                if (GetIfTable(pIfTable, &ulSize, FALSE) == NO_ERROR) {
                    ...
                    for (DWORD i=0; i < pIfTable->dwNumEntries; i++) {
                        ...
                        // Skip loopback
                        if (pIfRow->dwType != MIB_IF_TYPE_LOOPBACK && pIfRow->dwOperStatus == MIB_IF_OPER_STATUS_OPERATIONAL) {
